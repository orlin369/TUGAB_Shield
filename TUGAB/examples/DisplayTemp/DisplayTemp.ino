...
void loop()
{
  if(FilterCount < 20)
  {
    ...
    TmpSensorValue = TmpSensorValue + analogRead(SENSOR_TEMP);
    FilterCount++;
  }
  else
  {
    SensorValue = TmpSensorValue / 20;
    FilterCount = 0;
    TmpSensorValue = 0;
  }
  ...
  if (timerEvent)
  {
    ...
    {
      double celsius = Celsius(SensorValue);
      //float fahrenheit = celsius * (9.0 / 5.0) + 32.0;﻿
      SetIndicator(floor(celsius));
      delayValue = 0;
    }
    delayValue++;    
  }
  ...
  /*
  char printl[256];
  sprintf(printl, "RAW: %d, K: %f, C: %f: floor: %d\n",
              sensorValue, &kelvin, &celsius, ceilC);
